...
int main(int argc, char **argv){
    ...
    for (int w=0; w<worker; w++)
    {
        ...
        while (update < worker_update_times)
        {
            ...
            for (int b=0; b<num_negative; b++)
            {
                ...
                // gradient
                optimizer.feed_trans_margin_bpr_loss(mapper[user],
                                                     mapper[item],
                                                     item_pos_embed,
                                                     item_neg_embed,
                                                     8.0,
    ...
    */
    mapper.save_avg_to_file(&ui_file_graph, ui_file_graph.get_all_from_nodes(), save_name_q, 0);
    mapper.save_avg_to_file(&iu_file_graph, iu_file_graph.get_all_from_nodes(), save_name_q, 1);
    //mapper.save_meta_avg_to_file(&ui_file_graph, 1, up_file_graph, ui_file_graph.get_all_from_nodes(), save_name_q, 0);
    //mapper.save_meta_avg_to_file(&iu_file_graph, 1, im_file_graph, iu_file_graph.get_all_from_nodes(), save_name_q, 1);
